...
long getLineNumber(std::string const& fileName, const std::string& pathToSamtools) {
	string line ;
	long count = 0;
	FILE *stream;
    ...
    if (fileName.substr(fileName.size()-3,3).compare(".gz")==0) {
        ...
        string command = pathToSamtools + " view "+fileName;
...
void getBAFinfo(std::string BAFValuesInTheSegment,float copyNumber,float &estimatedBAF,float &fittedBAF,std::string &medianBAFSym, float & uncertainty, float normalContamination,int ploidy, bool noisyData, bool ifHomoz) {
    ...
    for (int unsigned i=0; i<copyNumbers.size(); i++) {
        ...
        if(myCopyNumber==0) {
            ...
            while (Bcount<=Acount) {
                ...
                double LogLikelyHood = calculateLogLikelyHoodNormalMixtureForBAFs(BAFs,mu,middleComponentMinWeight, fixedMu);
...
double calculateLogLikelyHoodNormalMixtureForBAFs(vector <float> X,vector <float> mu,float middleComponentMinW, bool isMuFixed) {
...
bool getSAMinfo(const char* line, std::string &chr1, std::string &chr2, char& orient1, char& orient2, int &left,int &right) {
    if (!*line) {
	  return false;
	}
	if (line[0] == '@') {
