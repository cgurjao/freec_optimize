...
void ChrCopyNumber::calculateRatioLog(ChrCopyNumber control, const double * a, const int degree){
	...
	for (int i = 0; i<length_; i++) {
		if ((control.getLength()>i)&&(control.getValueAt(i) != 0)){
			ratio_[i] = readCount_[i]/exp(polynomial(log(control.getValueAt(i)),a,1,degree));
			if (ratio_[i]<0)
				ratio_[i] = NA;
		} else
			if (readCount_[i]==0)
...
void ChrCopyNumber::calculateCopyNumberMedian(int ploidy, int minCNAlength, bool noisyData){ //create median profiles using 'bpfinal_' and store them in medianProfile_, info about medians themselves is stored in medianValues_ and about SD in sd_, lengths of fragments in bpLengths_
	if (ploidy!=ploidy_) {
	...
	for (int i  = 0; i < (int)bpfinal_.size();i++) {
        ...
        if (isBAFpresent && notNA > 100 && median < 1 && noisyData ) {
            ...
            double threshold = 0.0001*step_;
            if (1.*numberofBAFpoints/notNA < threshold)
                ifHomoz = true;
            cout <<"chr"<<chromosome_ <<":"<<breakPointStart<<"-"<<breakPointEnd<<" totalCount=" << totalCount << " notNA="<<notNA<<" numberofBAFpoints="<<numberofBAFpoints<<" ratio="<< 1.*numberofBAFpoints/notNA<<" hom:"<<ifHomoz<<"\n";
        }
...
//                        medianBAF = NA;
                    //medianBAF = NA;
                    getBAFinfo(BAFValuesInTheSegment,median*ploidy,estimatedBAF, fittedBAF, medianBAFSym,uncertainty, normalContamination_,ploidy_,noisyData,ifHomoz);
				}
			}
...
void ChrCopyNumber::addBAFinfo(SNPatChr SNPsatChr) {
